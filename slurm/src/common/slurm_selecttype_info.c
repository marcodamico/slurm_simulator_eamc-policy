...
int parse_select_type_param(char *select_type_parameters, uint16_t *param)
{
	...
	*param = 0;
	...
	while (str_parameters) {
		...
		} else if (!xstrcasecmp(str_parameters, "other_bsc_cons_res")) {
			*param |= CR_OTHER_BSC_CONS_RES;
...
extern char *select_type_param_string(uint16_t select_type_param)
{
	...
	if (select_type_param & CR_OTHER_BSC_CONS_RES) {
		if (select_str[0])
			strcat(select_str, ",");
		strcat(select_str, "OTHER_BSC_CONS_RES");
	}
