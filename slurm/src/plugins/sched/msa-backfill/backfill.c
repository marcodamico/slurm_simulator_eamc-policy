...
static bool _many_pending_rpcs(void)
{
	/* Marco: Simulator: time should not be passing here so no rpc should arrive */
#ifndef SLURM_SIMULATOR
...
next_task:
 ...
 TRY_LATER:
		...
		test_time_count++;
//Marco: I think we can avoid this part until backfill takes only one simulated second
#ifndef SLURM_SIMULATOR
		if (((defer_rpc_cnt > 0) &&
		     (slurmctld_config.server_thread_count >= defer_rpc_cnt)) ||
		    (slurm_delta_tv(&start_tv) >= sched_timeout)) {
...
//***************** Zia Edit Begin *******************************
		if(job_ptr->delayed_workflow) {
			debug("this workflow is delayed");
...
//***************** Zia Edit End *******************************
		...
		if ((job_ptr->start_time <= now) &&
			...
			if (bb == -1)
				continue;
			} else if (job_ptr->pack_job_id == 0 && job_ptr->start_time <= now) { /* Can start now */
			uint32_t save_time_limit = job_ptr->time_limit;
			uint32_t hard_limit;
...
//***************** Zia Edit Begin *******************************
		...
		if (rec->job_id == exclude_job_id || (rec->job_id != map->pack_job_id) || (rec->job_ptr->details && rec->job_ptr->details->delay))
//***************** Zia Edit End *******************************
			continue;
...
static int _test_workflow(pack_job_map_t *map, node_space_map_t *node_space)
{
	...
	while ((rec = (pack_job_rec_t *) list_next(iter)) && pack_start <= (now + backfill_window)) {
		...
		xfree(select_plugin);
		resv_msg.duration = (job_ptr->time_limit == INFINITE) ? YEAR_MINUTES : job_ptr->time_limit;
	...
	while ((rec = (pack_job_rec_t *) list_next(iter))) {
	  ...
	  prev_rec->job_ptr->details->delay = 0; //Also delays are not needed anymore 
	  prev_rec = rec;
	}
	prev_rec->job_ptr->details->delay = 0; //For the last record
