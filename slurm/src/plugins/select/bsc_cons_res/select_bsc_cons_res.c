/*****************************************************************************\
 *  select_bsc_cons_res.c - node selection plugin supporting consumable
 *  resources policies.
 *****************************************************************************\
 *
 ...
 */
const char plugin_name[] = "BSC Consumable Resources (CR) Node Selection plugin";
const char plugin_type[] = "select/bsc_cons_res";
const uint32_t plugin_id      = SELECT_PLUGIN_BSC_CONS_RES;
...
static void _create_part_data(void)
{
	...
	if (!num_parts)
		return;
	info("bsc_cons_res: preparing for %d partitions", num_parts);
			       ...
			       struct job_record *job_ptr)
{
	...
	debug3("bsc_cons_res: build_row_bitmaps reshuffling %u jobs", num_jobs);
	...
	if (j < num_jobs) {
		...
		debug3("bsc_cons_res: build_row_bitmap: dangling job found");
...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	debug3("bsc_cons_res: _add_job_to_res: job %u act %d ", job_ptr->job_id,
	       action);
	...
	for (i = i_first, n = -1; i <= i_last; i++) {
		...
		if (action != 2) {
			...
			if ((select_node_usage[i].alloc_memory >
			     select_node_record[i].real_memory)) {
				error("bsc_cons_res: node %s memory is "
	...
	if (action != 1) {
		...
		if (!p_ptr) {
			...
			error("bsc_cons_res: could not find cr partition %s",
			      part_name);
			return SLURM_ERROR;
		}
		if (!p_ptr->row) {
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!_can_job_fit_in_row(job, &(p_ptr->row[i])))
				continue;
			debug3("bsc_cons_res: adding job %u to part %s row %u",
		...
		if (i >= p_ptr->num_rows) {
			...
			error("bsc_cons_res: job overflow: "
		       ...
		       struct job_record *to_job_ptr)
{
	...
	if (from_job_ptr->job_id == to_job_ptr->job_id) {
		error("select/bsc_cons_res: attempt to merge job %u with self",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
	...
	if ((from_job_resrcs_ptr == NULL) ||
		...
		error("select/bsc_cons_res: job %u lacks a job_resources struct",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
			    ...
			    struct job_record *job_ptr, int action)
{
	...
	if (select_state_initializing) {
		/* Ignore job removal until select/bsc_cons_res data structures
	...
	debug3("bsc_cons_res: _rm_job_from_res: job %u action %d", job_ptr->job_id,
	       action);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
		_dump_job_res(job);
	...
	if (action != 1) {
		...
		if (!job_ptr->part_ptr) {
			error("bsc_cons_res: removed job %u does not have a "
			      "partition assigned",
			      job_ptr->job_id);
			return SLURM_ERROR;
		}
		...
		if (!p_ptr) {
			error("bsc_cons_res: removed job %u could not find part %s",
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			...
			for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
				if (p_ptr->row[i].job_list[j] != job)
					continue;
				debug3("bsc_cons_res: removed job %u from "
				       "part %s row %u",
				       job_ptr->job_id,
		...
		if (n) {
			...
			for (i = first_bit, n = -1; i <= last_bit; i++) {
				...
				if (node_usage[i].node_state >=
					...
					node_ptr = node_record_table_ptr + i;
					error("bsc_cons_res:_rm_job_from_res: "
				 ...
				 struct node_record *node_ptr)
{
	...
	if (!job || !job->core_bitmap) {
		error("%s: select/bsc_cons_res: job %u has no job_resrcs info",
		      __func__, job_ptr->job_id);
		return SLURM_ERROR;
	}

	debug3("bsc_cons_res: _rm_job_from_one_node: job %u node %s",
	       job_ptr->job_id, node_ptr->name);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
		_dump_job_res(job);
	...
	for (i = first_bit, n = 0; i <= last_bit; i++) {
		...
		if (node_usage[i].alloc_memory < job->memory_allocated[n]) {
			error("bsc_cons_res: node %s memory is underallocated "
	...
	if (!job_ptr->part_ptr) {
		error("bsc_cons_res: removed job %u does not have a partition "
	...
	for (i = 0; i < p_ptr->num_rows; i++) {
		...
		for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
			if (p_ptr->row[i].job_list[j] != job)
				continue;
			debug3("bsc_cons_res: found job %u in part %s row %u",
			       job_ptr->job_id, p_ptr->part_ptr->name, i);
	...
	if (n == 0) {
		error("bsc_cons_res: could not find job %u in partition %s",
	...
	if (node_usage[node_inx].node_state >= job->node_req) {
		...
		error("bsc_cons_res:_rm_job_from_one_node: node_state miscount");
		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	...
	if (job_ptr->part_ptr->cr_type) {
		if ((cr_type & CR_SOCKET) || (cr_type & CR_CORE)) {
			...
			info("bsc_cons_res: Can't use Partition SelectType unless "
 ...
 * the select/bsc_cons_res plugin data structures. */
		...
		while (more_jobs) {
			...
			while (true) {
				...
				if (overlap == 0)  /* job has no usable nodes */
					continue;  /* skip it */
				debug2("bsc_cons_res: _will_run_test, job %u: overlap=%d",
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	info("bsc_cons_res: select_p_node_init");
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: select_p_job_test: job %u node_req %u mode %d",
		     job_ptr->job_id, job_node_req, mode);
		info("bsc_cons_res: select_p_job_test: min_n %u max_n %u req_n %u "
		     "avail_n %u",
...
extern int select_p_reconfigure(void)
{
	...
	info("bsc_cons_res: select_p_reconfigure");
	select_debug_flags = slurm_get_debug_flags();
