			      ...
			      bool test_only, bitstr_t *part_core_map)
{
	...
	while (gres_cpus < cpus) {
		if ((int) cpus < cpu_alloc_size) {
			debug3("bsc_cons_res: cpu_alloc_size > cpus, cannot "
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: _can_job_run_on_node: %u cpus on %s(%d), "
		     "mem %"PRIu64"/%"PRIu64"",
	...
	for (i = i_first; i <= i_last; i++) {
		...
		if ((job_ptr->details->pn_min_memory) &&
			...
			if (select_node_record[i].real_memory >
				...
				free_mem = 0;
			if (free_mem < min_mem) {
				debug3("bsc_cons_res: _vns: node %s no mem %"
					""PRIu64" < %"PRIu64"",
			...
			if (node_usage[i].alloc_memory) {
				debug3("bsc_cons_res: _vns: node %s mem in use %"
					""PRIu64"",
					select_node_record[i].node_ptr->name,
					node_usage[i].alloc_memory);
				goto clear_bit;
		...
		if ((job_ptr->details->whole_node == 1) && exc_core_bitmap) {
			for (j = core_start_bit; j <= core_end_bit; j++) {
				if (bit_test(exc_core_bitmap, j))
					continue;
				debug3("bsc_cons_res: _vns: node %s exc",
				       select_node_record[i].node_ptr->name);
				goto clear_bit;
			}
		}
		...
		if (gres_cpus != NO_VAL)
			gres_cpus *= cpus_per_core;
		if (gres_cpus == 0) {
			debug3("bsc_cons_res: _vns: node %s lacks gres",
			       node_ptr->name);
			goto clear_bit;
		}
		...
		if (node_usage[i].node_state >= NODE_CR_RESERVED) {
			debug3("bsc_cons_res: _vns: node %s in exclusive use",
			       node_ptr->name);
			goto clear_bit;
		...
		/* non-resource-sharing node check */
			...
			if ((job_node_req == NODE_CR_RESERVED) ||
				...
				debug3("bsc_cons_res: _vns: node %s non-sharing",
				       node_ptr->name);
				goto clear_bit;
			}
			...
			if (_is_node_busy(cr_part_ptr, i, 1,
				...
				debug3("bsc_cons_res: _vns: node %s sharing?",
				       node_ptr->name);
				goto clear_bit;
			}
		...
		/* node is NODE_CR_AVAILABLE - check job request */
			...
			if (job_node_req == NODE_CR_RESERVED) {
				if (_is_node_busy(cr_part_ptr, i, 0,
						  job_ptr->part_ptr,
						  qos_preemptor)) {
					debug3("bsc_cons_res: _vns: node %s busy",
					       node_ptr->name);
					goto clear_bit;
				}
				...
				if (_is_node_busy(cr_part_ptr, i, 1,
						  job_ptr->part_ptr,
						  qos_preemptor)) {
					debug3("bsc_cons_res: _vns: node %s vbusy",
					       node_ptr->name);
					goto clear_bit;
				}
			}
			...
			bool prefer_alloc_nodes)
{
	...
	if (cr_node_cnt != node_record_count) {
		error("bsc_cons_res: node count inconsistent with slurmctld");
		return error_code;
	}
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		for (i = 0; i < consec_index; i++) {
			info("bsc_cons_res: eval_nodes:%d consec "
			     "c=%d n=%d b=%d e=%d r=%d",
			...
			bool qos_preemptor, bool preempt_mode)
{
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: evaluating job %u on %u nodes",
	...
	if (cpu_count == NULL) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 0 fail: "
		...
		xfree(cpu_count);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
			info("bsc_cons_res: cr_job_test: test 0 pass: test_only");
		return SLURM_SUCCESS;
	...
	xfree(cpu_count);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: test 0 pass - "
	...
	if ((cpu_count) && (job_ptr->best_switch)) {
		/* job fits! We're done. */
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 1 pass - "
	...
	if ((gang_mode == 0) && (job_node_req == NODE_CR_ONE_ROW)) {
		 ...
		 * before this point. */
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 1 fail - no idle resources available");
		}
		goto alloc_job;
	}
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: test 1 fail - "
	...
	if (!jp_ptr) {
		fatal("bsc_cons_res error: could not find partition for job %u",
			job_ptr->job_id);
	...
	if (preempt_by_part) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: looking for "
	...
	if (!cpu_count) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 2 fail - "
	...
	xfree(cpu_count);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: test 2 pass - "
	...
	if (cpu_count) {
		 ...
		 */
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
			info("bsc_cons_res: cr_job_test: test 3 pass - found resources");
		for (p_ptr = cr_part_ptr; p_ptr; p_ptr = p_ptr->next) {
			if (p_ptr->part_ptr->priority_tier >=
			    jp_ptr->part_ptr->priority_tier)
				continue;
			...
			if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
				info("bsc_cons_res: cr_job_test: remove low-priority partition %s",
				     p_ptr->part_ptr->name);
			}
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: test 3 fail - "
	...
	if (!jp_ptr || !jp_ptr->row) {
		...
		cpu_count = _select_nodes(job_ptr, min_nodes, max_nodes,
					  ...
					  prefer_alloc_nodes);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 4 pass - "
			     "first row found");
		}
		goto alloc_job;
	}
	...
	for (i = 0; i < c; i++) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
			info("bsc_cons_res: cr_job_test: test 4 fail - row %i", i);
	}
	...
	if (!cpu_count) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("bsc_cons_res: cr_job_test: test 4 fail - "
			     "busy partition");
		}
		goto alloc_job;
...
alloc_job:
	...
	if ((!cpu_count) || (!job_ptr->best_switch)) {
		...
		xfree(cpu_count);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
			info("bsc_cons_res: exiting cr_job_test with no allocation");
		return SLURM_ERROR;
	}
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: distributing job %u",
		     job_ptr->job_id);
	}
	...
	for (i = 0, n = 0; n < cr_node_cnt; n++) {
		...
		for (; j < k; j++, c++) {
			if (bit_test(free_cores, j)) {
				if (c >= csize)	{
					error("bsc_cons_res: cr_job_test "
	 ...
	 * job request.
	 * For bsc_cons_res, all available logical processors will be allocated on
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("bsc_cons_res: cr_job_test: job %u ncpus %u cbits "
