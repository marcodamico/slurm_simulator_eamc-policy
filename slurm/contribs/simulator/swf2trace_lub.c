...
int main(int argc, char* argv[])
{
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        i=0;
        while(p!=NULL){
            if(i==0) { printf("%s", p); job_arr[idx].job_id = atoi(p); job_arr[idx].wclimit = atoi(p);}   
            ...
            if(i==4) { printf("%s", p); job_arr[idx].tasks = ceil((float) atoi(p)); }   //Lublin does not consider CPUs per task
